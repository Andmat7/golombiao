<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
		...
		class test extends CI_Model {
			...
			public function new_team($location_team,$zone_team,$description_team,$name_team){


		$this->db->where('email', $name_team);
		$q = $this->db->get('name_team');
		if($q -> num_rows() == 1)
		{
			$user=$q->result_array();
			$output=false;
